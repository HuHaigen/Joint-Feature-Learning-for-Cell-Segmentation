...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.conv3_1 = conv3x3(inplanes, planes, stride)
        self.bn3_1 = nn.BatchNorm2d(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv3_2 = conv3x3(planes, planes)
        self.bn3_2 = nn.BatchNorm2d(planes)

        self.downsample = downsample
        self.stride = stride

        self.conv1_1 = conv1x1(inplanes, planes, stride)
        self.bn1_1 = nn.BatchNorm2d(planes)
        self.conv1_2 = conv1x1(planes, planes)
        self.bn1_2 = nn.BatchNorm2d(planes)

        self.conv5_1 = conv5x5(inplanes, planes, stride)
        self.bn5_1 = nn.BatchNorm2d(planes)
        self.conv5_2 = conv5x5(planes, planes)
    ...
    def forward(self, x):
        ...
        out_5 = self.conv5_1(x)
        out_5 = self.bn5_1(out_5)
        out_5 = self.relu(out_5)
        out_5 = self.conv5_2(out_5)
        out_5 = self.bn5_2(out_5)

        if self.downsample is not None:
            residual = self.downsample(x)

        out = out_1 + out_3 + out_5 + residual
        out = self.relu(out)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.bn1_1 = nn.BatchNorm2d(planes)
        self.conv1_2 = nn.Conv2d(planes, planes, kernel_size=1, stride=stride,padding=0, bias=False)
        self.bn1_2 = nn.BatchNorm2d(planes)
        self.conv1_3 = nn.Conv2d(planes, planes * 4, kernel_size=1, bias=False)
        ...
        self.conv3_1 = nn.Conv2d(inplanes, planes, kernel_size=1, bias=False)
        self.bn3_1 = nn.BatchNorm2d(planes)
        self.conv3_2 = nn.Conv2d(planes, planes, kernel_size=3, stride=stride,padding=1, bias=False)
        self.bn3_2 = nn.BatchNorm2d(planes)
        self.conv3_3 = nn.Conv2d(planes, planes * 4, kernel_size=1, bias=False)
        self.bn3_3 = nn.BatchNorm2d(planes * 4)

        self.conv5_1 = nn.Conv2d(inplanes, planes, kernel_size=1, bias=False)
        self.bn5_1 = nn.BatchNorm2d(planes)
        self.conv5_2 = nn.Conv2d(planes, planes, kernel_size=3, stride=stride,padding=1, bias=False)
        self.bn5_2 = nn.BatchNorm2d(planes)
        self.conv5_3 = nn.Conv2d(planes, planes * 4, kernel_size=1, bias=False)
        self.bn5_3 = nn.BatchNorm2d(planes * 4)
    ...
    def forward(self, x):
        ...
        out_1 = self.bn3(out_1)

        out_3 = self.conv1(x)
        out_3 = self.bn1(out_3)
        out_3 = self.relu(out_3)
        out_3 = self.conv2(out_3)
        out_3 = self.bn2(out_3)
        out_3 = self.relu(out_3)
        out_3 = self.conv3(out_3)
        out_3 = self.bn3(out_3)

        out_5 = self.conv1(x)
        out_5 = self.bn1(out_5)
        out_5 = self.relu(out_5)
        out_5 = self.conv2(out_5)
        out_5 = self.bn2(out_5)
        out_5 = self.relu(out_5)
        out_5 = self.conv3(out_5)
        out_5 = self.bn3(out_5)
