...
class ResidualUNet2D(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid, f_maps=16, conv_layer_order='cge', num_groups=8, model_deep=4,
                 **kwargs):
        super(ResidualUNet2D, self).__init__()

        if isinstance(f_maps, int):
            # use 5 levels in the encoder path as suggested in the paper
            f_maps = create_feature_maps(f_maps, number_of_fmaps=model_deep+1)
...
def main():
    model = ResidualUNet2D(1, 2, True, 16, 'cgr', 1)
    print(model)
    data = torch.zeros((1, 1, 96, 96))
    out = model(data)
    print(out.shape)
