...
def create_conv(in_channels, out_channels, kernel_size, order, num_groups, padding=1):
    ...
    for i, char in enumerate(order):
        if char == 'r':
            ...
            raise ValueError("Unsupported layer type '{}'. MUST be one of ['b', 'g', 'r', 'l', 'e', 'c']".format(char))
...
class Encoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, apply_pooling=True,
                 pool_kernel_size=(2, 2), pool_type='max', basic_module=DoubleConv, conv_layer_order='crg',
                 num_groups=8):
...
class FinalConv(nn.Sequential):
    """
    A module consisting of a convolution layer (e.g. Conv2d+ReLU+GroupNorm3d) and the final 1x1 convolution
    which reduces the number of channels to 'out_channels'.
    with the number of output channels 'out_channels // 2' and 'out_channels' respectively.
    We use (Conv2d+ReLU+GroupNorm3d) by default.
    This can be change however by providing the 'order' argument, e.g. in order
    to change to Conv2d+BatchNorm3d+ReLU use order='cbr'.
    Args:
        in_channels (int): number of input channels
        out_channels (int): number of output channels
        kernel_size (int): size of the convolving kernel
